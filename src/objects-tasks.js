...
function makeWord(lettersObject) {
  const result = [];
  Object.entries(lettersObject).forEach(([letter, arrValues]) => {
...
function sellTickets(queue) {
  const ticketCosts = 25;
  let moneyForChange = 0;

  return queue.every((bill) => {
    const changeNeeded = bill - ticketCosts;

    if (changeNeeded > moneyForChange) {
      return false;
    }

    moneyForChange += ticketCosts;
    moneyForChange -= changeNeeded;
    return true;
  });
}
